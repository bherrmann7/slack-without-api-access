...
- Exports at the bottom exist for tests, including a smoke test asserting every command and internal helper still resolves — a helper was once silently deleted by an edit and only surfaced on use. Add new helpers there. Most exports carry no stability promise; the browser plumbing (`launch`, `gotoClient`, `isSignedIn`, `dismissBanners`, `openConversation`, `log`, `die`, `STATE_DIR`) is the one surface a separate tool is expected to build on, so treat a rename there as a breaking change.
